...
int main () {
	...
	do { //loop que permite o usuário jogar novamente
	...
	printf("\n");
	if (memoria==chute){
		printf("Você já chutou esse número! Tente outra vez.");
		i--; 
	} else {
			...
			if (chute<0){
				...
				i--;
				continue; //Essa função interrompe e recomeça o loop. 
				         //É diferente da função break que sai completamente de loop;
			}
	    // a variável "acertou" recebe o valor 1 (verdadeiro) se a condição for verdadeira e 
		//recebe o valor 0 (falso) se a condição for falsa;
		...
		memoria=chute; //atualiza o valor da variável memória 
	}} //FIM DO LOOP
	...
	scanf("%d",&bis);
	} while (bis == 1); //Se o usuário digitar 1, o jogo recomeça.
